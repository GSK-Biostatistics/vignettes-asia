...
# Likelihood
  for (i in 1:N) {
    logit(p[i]) <- beta0[k_vec[i]] + beta1[k_vec[i]] * z_vec[i]
    y[i] ~ dbern(p[i])
  }
  ...
  for (k in 1:K) {
    ...
    beta0[k] <- mu0 + sigma0 * theta0[k]
    beta1[k] <- mu1 + sigma1 * theta1[k]
  }
  ...
  sigma0 ~ dnorm(0, tau) T(0,)
  sigma1 ~ dnorm(0, tau) T(0,)
  }
"
bhm_jags_mod <- jags.model(
  textConnection(BHM_jags_1),
  data = data_input_1,
  n.chains = 4,
  n.adapt = 10000
)
...
model {
  ...
  for (k in 1:K) {
    beta0[k] ~ dnorm(mu0, tau0)
    beta1[k] ~ dnorm(mu1, tau1)
  }
...
nimble_code_1 <- nimbleCode({  
  ...
  for (i in 1:N) {
    # Define the linear predictor separately
    p[i] <- ilogit(mu0 + sigma0 * beta0_tr[k_vec[i]] + 
                  (mu1 + sigma1 * beta1_tr[k_vec[i]]) * z_vec[i])
    y[i] ~ dbern(p[i])
  }

  for (k in 1:K) {
    beta0_tr[k] ~ dnorm(0, 1)
    beta1_tr[k] ~ dnorm(0, 1)
  }
  ...
  #Trying Half-Cauchy for the sigma parameters
  sigma0 ~ T(dt(0, 1, 1), 0, ) #(A t-distribution with 1 degree of freedom is a Cauchy)
  sigma1 ~ T(dt(0, 1, 1), 0, )
  }
...
inits <- list(
  ...
  sigma0 = 1,
  sigma1 = 1,
  beta0 = rep(0, data_input_1$K),
  beta1 = rep(0, data_input_1$K)
)
