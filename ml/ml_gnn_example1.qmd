---
title: "Tensorflow GNN Example - MUTAG Molecule Data"
output: 
...
This example is ported from the [original on Tensorflow GNN github site](https://colab.research.google.com/github/tensorflow/gnn/blob/master/examples/notebooks/intro_mutag_example.ipynb). The modelling objective here is to predict a classification for each molecule structure, for example is a given molecule structure more likely to be mutagenic or not. Unlike other vignettes, a custom Python setup is required here for reticulate so as ensure correct version compatibility of tensorflow and tensorflow-gnn. See the [installation guide](https://gsk-biostatistics.github.io/vignettes-asia/tfp/index_tfp.html).
...
train_path = os.path.join(os.path.expanduser('~'), 'data', 'mutag','train.tfrecords')
val_path = os.path.join(os.path.expanduser('~'), 'data', 'mutag', 'val.tfrecords')
