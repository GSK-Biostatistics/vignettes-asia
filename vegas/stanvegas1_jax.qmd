---
title: "Logistic BHM - Vegas JAX v RStan"
output:
...
install_if_missing(c("numpy","pandas","jax","vegas","gvar"))
...
```{python sixvarjaco,include=FALSE,eval=TRUE}
import numpy as np
import pandas as pd
...
def compute_log_lik(theta, y,T):
    ...
    jacobianL = (  jnp.log1p(jnp.power(theta0,2)) - 2.0*jnp.log1p(-jnp.power(theta0,2)) 
                 + jnp.log1p(jnp.power(theta1,2)) - 2.0*jnp.log1p(-jnp.power(theta1,2))
...
```{python margintercept,include=FALSE,eval=TRUE}
